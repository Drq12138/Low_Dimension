...
def main():
    ...
    optimizer.zero_grad()
    print ('grad accumulate:', args.accumulate)
...
def P_plus_BFGS(model, optimizer, grad, oldf, X, y):
    ...
    gk = torch.mm(P, grad.reshape(-1,1))                # gk: [40,1]/[n_low_dimension,1]

    grad_proj = torch.mm(P.transpose(0, 1), gk)         # grad_proj: [269722, 1]/[n_parameter, 1]
    grad_res = grad - grad_proj.reshape(-1)             # grad_res: [269722]
