...
def build_new_model(P, conv_names, conv_index, conv_shapes, origin_weight):

    return resnet.__dict__['replace_resnet20'](P, conv_names, conv_index, conv_shapes, origin_weight)



def divide_param(model, all=False):
    ...
    if all:
        for name, param in model.named_parameters():
            end_count = index_count + param.data.numel()
            
            conv_names.append(name)
            conv_index.append((index_count, end_count))
            conv_shapes.append(param.shape)
        
            index_count = end_count
    else:
        for name, param in model.named_parameters():
            end_count = index_count + param.data.numel()
            ...
            index_count = end_count
    return conv_names, conv_index, conv_shapes
